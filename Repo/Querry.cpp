...
string Querry::GetQuerry(const querryType& querryType,const string& maSv,const string& className) {
    switch (querryType) {
        case querryType::InsertDiemDanh:
              return "CALL proc_diem_danh_tong_hop('"+maSv+"')";
        case querryType::KiemTraSinhVien: {
            return "SELECT user_name,class_name,full_name,DATE(modified_date) AS modified_date FROM student WHERE user_name = '" + maSv +"'";
        }
        default:
            return "";
    }
